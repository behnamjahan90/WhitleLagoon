...
namespace WhiteLagoon.Application.Services.Implementation
{
    public class DashboardService : IDashboardService
    {
        ...
        readonly static int previousMonth = DateTime.Now.Month == 1 ? 12 : DateTime.Now.Month - 1;
        ...
        public async Task<PieChartDto> GetBookingPieChartData()
        {
            ...
            var ss = totalBookings.GroupBy(u => u.UserId);
            var s2 = ss.Where(x => x.Count() == 1);
            var s3 = s2.Select(x => x.Key);

            var customerWithBooking = totalBookings.GroupBy(u => u.UserId).Where(x => x.Count() == 1).Select(x => x.Key).ToList();//key in this section means userId

            int bookingsByNewCustomer = customerWithBooking.Count();
        ...
        public async Task<RadialBarChartDto> GetRegisteredUserChartData()
        {
            ...
            return SD.GetRadialChartDataModel(totalUsers.Count(), countByCurrentMonth, countByPrevioustMonth);
        }
        public async Task<RadialBarChartDto> GetRevenueChartData()
        {
           ...
           && u.Status != SD.StatusCanceled);//need to be check
            ...
            var countByPrevioustMonth = totalBookings.Where(u => u.BookingDate >= previousMonthStartDate &&
            u.BookingDate <= currentMonthStartDate).Sum(u => u.TotalCost);


            return SD.GetRadialChartDataModel(totalRevenue, countByCurrentMonth, countByPrevioustMonth);
        }
        public async Task<RadialBarChartDto> GetTotalBookingRadialChartData()
        {
            ...
            var countByPrevioustMonth = totalBookings.Count(u => u.BookingDate >= previousMonthStartDate &&
            u.BookingDate <= currentMonthStartDate);

            return SD.GetRadialChartDataModel(totalBookings.Count(), countByCurrentMonth, countByPrevioustMonth);
        }
